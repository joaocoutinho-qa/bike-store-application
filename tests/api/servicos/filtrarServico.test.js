...
describe('Filtrar serviço específico', () => {
    it('Deve filtrar um serviço com sucesso', async () => {
        ...
        const resposta = await request(URL_DA_API)
            .get(`/servicos/${id_servico}`)
            .set('Authorization', `Bearer ${token}`);
        expect(resposta.statusCode).to.equal(200);
    });

    it('Não deve filtrar um serviço com ID inválido', async () => {
        const resposta = await request(URL_DA_API)
            .get(`/servicos/ID_inválido`)
            .set('Authorization', `Bearer ${token}`);
